...
export function ClienteForm({ initial, onSubmit }: { initial?: Cliente; onSubmit: (data: ClienteFormData) => void | Promise<void> }) {
  const { register, handleSubmit, formState: { errors } } = useForm<ClienteFormData>({
    ...
    defaultValues: initial
      ? { nombre: initial.nombre, telefono: initial.telefono, localidad: initial.localidad ?? "", notas: initial.notas ?? "" }
      : { nombre: "", telefono: "", localidad: "", notas: "" },
  });
  ...
  return (
    <form className="grid gap-4" onSubmit={handleSubmit(onSubmit)}>
      ...
      <Field label="Localidad"><Input placeholder="Ej. Ciudad Guzman, Zapotiltic, Tuxpan..." {...register("localidad")} /></Field>
